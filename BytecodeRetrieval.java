...
class BytecodeRetrieval implements Serializable {
  ...
  BytecodeRetrieval(final HashSet _hs, WVM wvm,
    ...
    WorkletJunction targetWJ = new TargetWJ(rHost, rName, rPort, _hs);
    WVM.out.println("da BytecodeRetrieval: " + targetWJ);

    wkl.addJunction(targetWJ);
...
class OriginWJ extends WorkletJunction {
  ...
  public void execute() {
    ...
    while (keys.hasMoreElements()) {
      ...
      Hashtable ht = new Hashtable(); // Hashtable ht = (psl.worklets.http.ClassServer).bytecodeCache;
